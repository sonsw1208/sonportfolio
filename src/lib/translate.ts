...
// Bump version khi đổi cách hậu xử lý bản dịch — cache cũ trong localStorage sẽ bị bỏ qua,
// nếu không người dùng vẫn thấy bản dịch lỗi đã lưu từ trước.
const TR_KEY = "trCacheVienEn_v2";
...
function loadTR() {
  ...
  trLoaded = true;
  try {
    localStorage.removeItem("trCacheVienEn_v1"); // dọn cache phiên bản cũ, không còn dùng
...
// MyMemory hay thêm dấu câu không có trong bản gốc — VD "Năm kinh nghiệm" → "Years of
// experience:". Chỉ cắt dấu ở CUỐI và chỉ khi bản gốc không có dấu đó, để không đụng vào
// câu vốn kết thúc bằng "." hay "?".
function stripAddedPunctuation(src: string, translated: string): string {
  const trailing = /[:;,.!?]+$/;
  if (trailing.test(src)) return translated;
  return translated.replace(trailing, "");
}
...
async function trSeg(seg: string): Promise<string> {
  ...
  try {
    ...
    if (t && typeof t === "string" && String(j.responseStatus) === "200") {
      const clean = stripAddedPunctuation(k, t.trim());
      TR[k] = clean;
      trDirty = true;
      return seg.replace(k, clean);
    }
  } catch {
